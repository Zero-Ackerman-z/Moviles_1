        ...
        private void Update()
        {
            if (Input.touchCount > 0)
            {
                ...
                if (touch.phase == TouchPhase.Ended || touch.phase == TouchPhase.Canceled)
                {
                    ...
                    if (isSwiping && !isDragging && swipeDistance > 200f)
                    {
                        DeleteAllObjects();
                    }
                    else if (!isDragging && !IsTouchOverUI(touch) && swipeDistance < 50f)
                    {
        ...
        private void CreateTrail(Vector3 position)
        {
            if (trailPrefab != null && activeTrail == null)
            {
                ...
                if (trail != null)
                {
                    ...
                    trail.endColor = new Color(GameConfigManager.Instance.selectedColor.r, GameConfigManager.Instance.selectedColor.g, GameConfigManager.Instance.selectedColor.b, 0f); 
                }
            }
        }
    }
